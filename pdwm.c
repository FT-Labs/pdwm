...
static const Key defkeys[] = { STACKKEYS(Mod4Mask, focus) STACKKEYS(
	Mod4Mask | ShiftMask, push) TAGKEYS(XK_1, 0) TAGKEYS(XK_2, 1) TAGKEYS(XK_3, 2)
...
void swallow(Client *p, Client *c)
{
	...
	if (!swallowfloating && c->isfloating) return;
		...
		do {
			if ((1 << i) & c->tags) {
				...
				if (selmon->sel != c || HIDDEN(c)) {
					...
					focus(c);
					if (!c->isfloating)
						XWarpPointer(dpy, None, c->win, 0, 0, 0,
							     0, c->w / 2, c->h / 2);
					restack(selmon);
				}
			}
				...
				for (c = m->clients; c; c = c->next)
					if (c->isfullscreen)
						resizeclient(c, m->mx, m->my, m->mw,
							     m->mh);
				XMoveResizeWindow(dpy, m->barwin, m->wx + sb_padding_x,
						  m->by, m->ww - 2 * sb_padding_x, bh);
			}
			focus(NULL);
			arrange(NULL);
		}
...
{
	if (!m->barwin || !allbarwin[0] || !allbarwin[1]) return;
	...
	if (showsystray) stw = getsystraywidth();
	...
	if (m == selmon) {
		...
		XMoveResizeWindow(
			...
			m->by, stw + twtmp - (showsystray ? 0 : 2 * sb_padding_x), bh);
		if (showsystray)
		...
		while (sb_arr[i] != NULL) {
			...
			if (strlen(sb_arr[i]) == 1) {
				...
				drw_pic(drw, m->ww - twtmp, (bh - sb_icon_wh) / 2,
					sb_icon_wh, sb_icon_wh, None, idx);
				twtmp -= sb_icon_wh + sb_icon_margin_x;
				drw_text(drw, m->ww - twtmp, y, TEXTW_SB(sb_arr[i]), bh,
					 0, sb_arr[i], 0);
			...
			if (sb_arr[i + 1] != NULL || showsystray) {
				...
				drw_rect(drw, m->ww - twtmp + sb_delimiter_w / 4, y,
					 sb_delimiter_w / 2, bh, 1, 0);
	...
	drw_pic(drw, sb_delimiter_w, y + (bh - sb_icon_wh) / 2, sb_icon_wh, sb_icon_wh,
		None, 0);
	x += sb_icon_wh + 2 * sb_delimiter_w;
	...
	if ((w = m->ww - tw - x - stw - 2 * sb_padding_x) > bh) {
		...
		if (m->sel && m == selmon) {
			...
			s = drw_text(drw, x, 0,
			    ...
			    x;
			if (c->icon)
				drw_pic(drw, x + lrpad / 2, (bh - c->ich) / 2, c->icw,
					c->ich, c->icon, -1);
			if (c->isfloating)
				drw_rect(drw, x + boxs, boxs, boxw, boxw, c->isfixed, 0);
			XMoveResizeWindow(dpy, allbarwin[0],
					  m->wx +
						  (m->bleftend + m->brightstart - s -
						   stw - sb_padding_x) /
							  2 +
						  2 * sb_padding_x,
	...
	if (m == selmon) drw_map(drw, allbarwin[1], m->brightstart, 0, m->ww, bh);
}
...
void focusstack(const Arg *arg)
{
	if (!selmon->sel || !selmon->clients) return;
	Client *c = NULL, *i;
...
Atom getatomprop(Client *c, Atom prop)
{
	...
	if (XGetWindowProperty(dpy, c->win, prop, 0L, sizeof atom, False, req, &da, &di,
			       &dl, &dl, &p) == Success &&
	    p) {
		atom = *(Atom *)p;
...
void layoutmenu(const Arg *arg)
{
	...
	if (!(p = popen(layoutmenu_cmd, "r"))) return;
...
void movemouse(const Arg *arg)
{
	...
	do {
		...
		case Expose:
		case MapRequest:
			if ((ev.xmotion.time - lasttime) <= (1000 / 280)) continue;
			handler[ev.type](&ev);
			break;
		case MotionNotify:
			if ((ev.xmotion.time - lasttime) <= (1000 / 280)) continue;
...
int sendevent(Window win, Atom proto, int m, long d0, long d1, long d2, long d3, long d4)
{
	int n;
	Atom *protocols, mt;
	int exists = 0;
...
void setup(void)
{
	...
	if (!drw_fontset_create(drw, fonts, lenfonts)) die("no fonts could be loaded.");
	lrpad = drw->fonts->h;
...
void updatebars(void)
{
	...
	for (int i = 0; i < 2; i++) {
		if (!allbarwin[i]) {
			allbarwin[i] = XCreateWindow(
				dpy, root, 0, 0, bh, bh, 0, DefaultDepth(dpy, screen),
				CopyFromParent, DefaultVisual(dpy, screen),
				CWColormap | CWOverrideRedirect | CWBackPixmap |
					CWEventMask,
				&wa);
	...
	for (m = mons; m; m = m->next) {
		if (!m->barwin) {
			m->barwin =
				XCreateWindow(dpy, root, m->wx + sb_padding_x, m->by,
					      m->ww - 2 * sb_padding_x, bh, 0,
...
int main(int argc, char *argv[])
{
	...
	if (!(dpy = XOpenDisplay(NULL))) die("pdwm: cannot open display");
	if (!(xcon = XGetXCBConnection(dpy))) die("pdwm: cannot get xcb connection\n");
	checkotherwm();
	buttons = get_buttons();
	keys = get_keys();
	fonts = get_fonts();
